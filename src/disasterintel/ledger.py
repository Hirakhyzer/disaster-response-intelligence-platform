"""Hash-chained audit ledger for disaster-response experiments."""
...
def append_record(path: str | Path, record: dict[str, Any]) -> dict[str, Any]:
    """Append one linked JSONL audit record and return the enriched payload."""
...
def verify_ledger(path: str | Path) -> dict[str, Any]:
    """Verify continuity and integrity of a JSONL experiment ledger."""
    destination = Path(path)
    previous_hash = None
    count = 0
